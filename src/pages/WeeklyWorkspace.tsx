...
export function WeeklyWorkspace() {
  ...
  const steps = [
    ...
    { label: "证据", detail: "2 项缺口待处理", done: false, to: "/proofs?readiness=gap" },
    { label: "风险", detail: "2 条需要确定性门禁", done: false, to: "/content?status=review" },
    { label: "审批", detail: "2 项正在等待负责人", done: false, to: "/execution?tab=approvals" },
    { label: "发布", detail: "必须人工复制发布", done: false, to: "/content?status=ready" },
    { label: "结果", detail: "48 小时内回填", done: false, to: "/execution?tab=tasks" },
  ];
  async function regenerate() {
    ...
    try {
      ...
      const result = await aiClient.weeklyStrategy({
        ...
        metrics: { ready_drafts: ready, pending_approvals: currentState.approvals.filter((item) => item.status === "pending").length, overdue_tasks: 2 },
  ...
  return <>
    ...
    {!health?.ai_configured && <InlineAlert tone="warning" title="AI 生成当前被阻断">本地 BFF 未检测到 `OPENAI_API_KEY`。现有输入与策略不会被覆盖，配置后可直接重试。</InlineAlert>}
    {error && <InlineAlert tone="danger" title={`生成失败 · ${error.code}`}>{error.message} <button className="text-button" onClick={() => void regenerate()}>重试</button></InlineAlert>}
    <section className="workflow-strip" aria-label="本周内容工作流">{steps.map((step, index) => <div className={`workflow-step ${step.done ? "complete" : "blocked"}`} key={step.label}><div className="workflow-line"><span>{step.done ? <Check /> : <CircleDashed />}</span>{index < steps.length - 1 && <i />}</div><b>{step.label}</b><small>{step.detail}</small><Link to={step.to}>打开<ArrowRight /></Link></div>)}</section>
    <div className="weekly-layout" id="strategy">
      <section className="panel strategy-panel"><div className="panel-heading"><div><span className="eyebrow">当前策略</span><h2>{state.weekly_plan.strategy.theme}</h2></div><span className="version-label">{state.weekly_plan.generated_by}</span></div><p className="strategy-objective">{state.weekly_plan.strategy.objective}</p>
      ...
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">本周排期</span><h2>内容 Slot</h2></div><span className="count-badge">{state.weekly_plan.strategy.content_slots.length}</span></div><div className="slot-list">{state.weekly_plan.strategy.content_slots.map((slot) => <div key={`${slot.day}-${slot.topic}`}><span className={`stage-code stage-${slot.stage.toLowerCase()}`}>{slot.stage}</span><span><strong>{slot.day} · {slot.topic}</strong><small>唯一 CTA：{slot.cta}</small></span></div>)}</div></section>
      <aside className="panel gate-panel"><div className="panel-heading"><div><span className="eyebrow">阻塞原因</span><h2>核心 Gate</h2></div><LockKeyhole /></div><ul className="check-list"><li><b>证据</b><span>D1 同业证明不足，1 项授权已撤销</span></li><li><b>审批</b><span>客户证明与量化结果需负责人处理</span></li><li><b>外部动作</b><span>仅允许复制草稿，禁止自动外发</span></li></ul><Link className="secondary-button full" to="/execution">进入处理队列 <ArrowRight /></Link></aside>
    </div>
