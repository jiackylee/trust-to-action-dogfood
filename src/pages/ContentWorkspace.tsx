...
export function ContentWorkspace() {
  const { state, loading, health, generateMarketingCandidate, saveDraft, submitDraftApproval, markPublished, explainError, notify } = useAppStore();
  ...
  const marketingCandidate = state.marketing_candidates.find((item) => item.task_type === "content_draft" && item.subject_id === currentDraft.id && item.status === "pending");
  ...
  async function generateDraft() {
    ...
    try {
      await generateMarketingCandidate("content_draft", currentDraft.id, currentDraft.revision, `${currentDraft.title} ${currentDraft.segment} 朋友圈草稿 品牌语气 证明 CTA`, { draft_id: currentDraft.id });
  ...
  return <>
    ...
    {!health?.knowledge_configured && <InlineAlert tone="warning" title="知识增强未就绪">知识包未配置或未激活，草稿候选会明确阻断，不会回退到通用 Prompt。</InlineAlert>}
    <div className="content-workspace desktop-editor">
      ...
      <section className="editor-pane">
        <div className="editor-toolbar"><div className="segmented-control" aria-label="漏斗阶段">{["T", "I", "D", "A"].map((stage) => <button disabled={!canEdit} className={local.stage === stage ? "active" : ""} onClick={() => change("stage", stage as Draft["stage"])} key={stage}>{stage}</button>)}</div><div><button className="secondary-button" onClick={() => marketingCandidate ? setCandidateOpen(true) : void generateDraft()} disabled={Boolean(generating) || !canEdit} title={!canEdit ? "只有运营角色可以编辑草稿" : undefined}><Sparkles />{generating === "draft" ? "生成中" : marketingCandidate ? "审阅 AI 候选" : "AI 生成候选"}</button><button className="icon-button" title="立即保存" aria-label="立即保存" disabled={!dirty || !canEdit} onClick={() => void saveCurrent()}><Save /></button></div></div>
    ...
    <div className="mobile-draft-list">{drafts.map((draft) => { const pendingCandidate = state.marketing_candidates.find((item) => item.task_type === "content_draft" && item.subject_id === draft.id && item.status === "pending"); return <article key={draft.id} className="mobile-card"><div className="mobile-card-head"><span className={`stage-code stage-${draft.stage.toLowerCase()}`}>{draft.stage}</span><StatusBadge status={draft.status} /></div><h2>{draft.title}</h2><p>{draft.body.slice(0, 88)}…</p><dl><div><dt>目标</dt><dd>{draft.segment}</dd></div><div><dt>行动指引</dt><dd>{draft.cta}</dd></div></dl>{draft.approval_required && <span className="warning-line"><ShieldCheck />需要负责人审批</span>}{pendingCandidate && <button className="secondary-button full" onClick={() => { setSelectedId(draft.id); setLocal(draft); setCandidateOpen(true); }}><Sparkles />审阅 AI 候选</button>}</article>; })}</div>
    ...
    <Modal open={candidateOpen} title="检查 AI 草稿候选" onClose={() => setCandidateOpen(false)}>{marketingCandidate ? <MarketingDecisionPanel candidate={marketingCandidate} currentRevision={currentDraft.revision} compact onDecision={() => setCandidateOpen(false)} /> : <InlineAlert tone="info" title="候选正在刷新">生成完成后将显示业务证据、知识依据、SKILL 路由和可审阅正文。</InlineAlert>}</Modal>
